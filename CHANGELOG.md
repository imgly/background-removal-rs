...
- **Dual stacked progress bars**: When `--progress` flag is used, displays two visually stacked progress bars - one showing total files/items progress and another showing current file processing stages. For video processing, shows real-time frame-by-frame progress with determinate progress bar displaying current frame and total frames
- **Video frame progress tracking**: Added frame count to `VideoMetadata` and implemented progress callback mechanism in `RemovalConfig` to enable real-time frame-by-frame progress reporting during video processing
