...
fn create_test_image_with_profile(
    dir: &TempDir,
    format: &str,
...
async fn test_jpeg_color_profile_preservation() -> Result<()> {
    ...
    println!(
        "Test setup complete - would verify JPEG color profile preservation with real processor"
    );
...
async fn test_color_profile_preservation_disabled() -> Result<()> {
    ...
    println!(
        "Test setup complete - would verify processing works without color profile preservation"
    );
...
fn test_color_profile_extraction_api() -> Result<()> {
    ...
    if let Some(profile) = profile_result {
        ...
        println!(
            "Successfully extracted profile: {} ({} bytes)",
            profile.color_space,
            profile.data_size()
        );
    } else {
...
fn test_color_profile_embedding_api() -> Result<()> {
    ...
    assert!(
        result.is_ok(),
        "Embedding with empty profile should succeed"
    );
...
fn test_removal_result_color_profile_handling() -> Result<()> {
    ...
    use imgly_bgremove::types::{
        ColorProfile, ProcessingMetadata, RemovalResult, SegmentationMask,
    };
    ...
    let result_no_profile = RemovalResult::new(image, mask, (8, 8), metadata);
...
fn test_stream_processing_fix_validation() {
    ...
    use imgly_bgremove::models::{ModelSource, ModelSpec};
