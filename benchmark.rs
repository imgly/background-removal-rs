//! Comprehensive benchmark for model precision and execution provider performance
//!
//! Tests all combinations of:
//! - Model precision: FP32 vs FP16
//! - Execution providers: CPU vs CoreML (Apple Silicon GPU)
...
impl BenchmarkSuite {
    ...
    fn print_summary(&self) {
        ...
        for (size, results) in by_size {
            ...
            for (rank, result) in sorted_results.iter().enumerate() {
                ...
                println!("{} {:20} | {:6.1}ms | {:6.1}-{:6.1}ms | {:5.1}% success",
                    medal,
                    result.config.name,
                    result.avg_time_ms,
                    result.min_time_ms,
    ...
    fn print_analysis(&self) {
        ...
        if let Some(fastest) = self.results.iter().min_by(|a, b| a.avg_time_ms.partial_cmp(&b.avg_time_ms).unwrap()) {
            println!("🚀 Fastest Overall: {} ({:.1}ms average)", fastest.config.name, fastest.avg_time_ms);
        }
        
        // Note: Model precision comparison removed since precision is now handled automatically
        
        // Compare by provider
        let cpu_avg = self.results.iter()
            .filter(|r| matches!(r.config.provider, ExecutionProvider::Cpu))
            .map(|r| r.avg_time_ms)
            .collect::<Vec<_>>();
        let coreml_avg = self.results.iter()
...
async fn benchmark_configuration(
    ...
    if times.is_empty() {
        return Err(anyhow::anyhow!("No successful iterations for {}", config.name));
    }
    ...
    println!("  ✅ Completed: {:.1}ms avg ({:.1}% success)", avg_time, success_rate * 100.0);
...
async fn main() -> Result<()> {
    ...
    println!("Testing CPU/CoreML execution providers");
    println!("═══════════════════════════════════════════════════════════════\n");
    ...
    for (width, height, pattern) in test_cases {
        println!("📸 Generating test image: {}x{} ({})", width, height, pattern);
