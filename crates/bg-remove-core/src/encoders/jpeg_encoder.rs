...
impl JpegIccEncoder {
    ...
    pub fn encode_with_profile<P: AsRef<Path>>(
        ...
        let mut temp_jpeg = Vec::new();
        {
            ...
            encoder
                ...
                .map_err(|e| BgRemovalError::processing(format!("Failed to encode JPEG: {e}")))?;
        }
    ...
    fn embed_icc_in_jpeg(jpeg_data: &[u8], icc_data: &[u8]) -> Result<Vec<u8>> {
        ...
        if jpeg_data.len() < 2
            || jpeg_data.first() != Some(&0xFF)
            || jpeg_data.get(1) != Some(&0xD8)
        {
    ...
    fn create_icc_app2_segments(icc_data: &[u8]) -> Result<Vec<Vec<u8>>> {
        ...
        let total_segments =
            (icc_data.len() + MAX_ICC_DATA_PER_SEGMENT - 1) / MAX_ICC_DATA_PER_SEGMENT;
        ...
        let total_segments: u8 = total_segments.try_into().map_err(|_| {
            BgRemovalError::processing(
                "ICC profile too large - requires more than 255 JPEG segments",
            )
        })?;

        for (chunk_index, icc_chunk) in icc_data.chunks(MAX_ICC_DATA_PER_SEGMENT).enumerate() {
            let sequence_number: u8 = (chunk_index + 1).try_into().map_err(|_| {
                BgRemovalError::processing("JPEG segment sequence number too large for u8")
            })?;

            let segment_length_usize = ICC_IDENTIFIER.len() + 1 + 1 + icc_chunk.len() + 2;
            let segment_length: u16 = segment_length_usize.try_into().map_err(|_| {
                BgRemovalError::processing("JPEG ICC segment length too large for u16")
            })?;
...
mod tests {
    ...
    fn test_create_large_icc_segments() {
        ...
        for (i, segment) in segments.iter().enumerate() {
            let sequence_num: u8 = (i + 1)
                .try_into()
                .expect("Test segment index should fit in u8");
            let total_segments: u8 = segments
                .len()
                .try_into()
                .expect("Test segment count should fit in u8");
