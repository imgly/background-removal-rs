...
async fn test_timing_measurements_integration() {
    ...
    assert!(timings.total_ms >= timings.inference_ms, "Total time should be >= inference time");
    assert!(timings.total_ms >= timings.preprocessing_ms, "Total time should be >= preprocessing time");
    assert!(timings.total_ms >= timings.postprocessing_ms, "Total time should be >= postprocessing time");
    
    // Verify breakdown adds up (allowing for rounding)
    let breakdown = timings.breakdown_percentages();
    let total_percentage = breakdown.decode_pct + breakdown.preprocessing_pct + 
                          breakdown.inference_pct + breakdown.postprocessing_pct + 
                          breakdown.encode_pct + breakdown.other_pct;
...
fn test_timing_invariants() {
    ...
    let measured_sum = timings.image_decode_ms + timings.preprocessing_ms + 
                      timings.inference_ms + timings.postprocessing_ms + 
                      timings.image_encode_ms.unwrap_or(0);
    
    timings.total_ms = measured_sum + 5; // 5ms overhead
    
    // Test invariants
    assert!(timings.total_ms >= measured_sum, "Total time should be >= sum of measured phases");
    assert!(timings.other_overhead_ms() == 5, "Overhead calculation should be correct");
...
async fn test_timing_overhead() {
    ...
    assert!(overhead_percentage < 5.0, 
           "Timing overhead should be < 5% of total time, got {:.1}%", overhead_percentage);
}