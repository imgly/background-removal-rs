...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_images = [
        ("Portrait", "crates/bg-remove-testing/assets/input/portraits/portrait_single_simple_bg.jpg"),
        ("Product", "crates/bg-remove-testing/assets/input/products/product_clothing_white_bg.jpg"),
        ("Complex", "crates/bg-remove-testing/assets/input/complex/complex_group_photo.jpg"),
        ("Multiple People", "crates/bg-remove-testing/assets/input/portraits/portrait_multiple_people.jpg"),
    ];
    
    // Filter to only existing images
    let available_images: Vec<(&str, &str)> = test_images.iter()
    ...
    println!("   • Size difference: {:.1}x larger (FP32)", fp32_size / fp16_size);
    ...
    println!("   ─────────────────────────────────────────────────────────");
    println!("   Avg Processing Time   {:.0}ms       {:.0}ms       {:.1}x {}",
        ...
        if fp32.avg_processing_time_ms > fp16.avg_processing_time_ms { "slower" } else { "faster" }
    );
    
    println!("   Avg Inference Time    {:.0}ms       {:.0}ms       {:.1}x {}",
        ...
        if fp32.avg_inference_time_ms > fp16.avg_inference_time_ms { "slower" } else { "faster" }
    );
    
    println!("   Inference Ratio       {:.1}%        {:.1}%        {:.1}% diff",
        fp16.inference_ratio * 100.0, fp32.inference_ratio * 100.0,
        (fp32.inference_ratio - fp16.inference_ratio) * 100.0
    );
    
    println!("\n💾 Resource Comparison:");
    println!("   Model Size            {:.1}MB       {:.1}MB       {:.1}x larger",
        fp16.model_size_mb, fp32.model_size_mb, fp32.model_size_mb / fp16.model_size_mb);
    ...
    let performance_diff = (fp32.avg_processing_time_ms - fp16.avg_processing_time_ms) / fp16.avg_processing_time_ms * 100.0;
    ...
    if performance_diff > 5.0 {
        ...
        println!("   • Performance difference is negligible ({:.1}%)", performance_diff.abs());
    }
    
    println!("   • FP32 model is {:.1}x larger ({:.0}MB vs {:.0}MB)", 
    ...
    if performance_diff.abs() < 5.0 {
        ...
        println!("   📝 Reasoning: Similar performance with {:.1}x smaller binary size", size_ratio);
        println!("   💡 Users prioritizing download speed and storage will benefit");
        println!("   🔧 FP32 can be enabled via --features fp32-model for accuracy-critical use cases");
    } else if performance_diff > 10.0 {
        // FP32 significantly slower
        println!("   ✅ KEEP FP16 as default");
        println!("   📝 Reasoning: FP16 is {:.1}% faster with {:.1}x smaller size", -performance_diff, size_ratio);
        ...
        println!("   📝 Reasoning: FP32 is {:.1}% faster (trade-off vs {:.1}x larger size)", -performance_diff, size_ratio);
    ...
    println!("   • Relative to typical application size: {}",
        if size_diff_mb > 50.0 { "Significant impact" } 
        else if size_diff_mb > 20.0 { "Moderate impact" }
        else { "Minor impact" });
    ...
    for (image_name, path) in test_images {
        ...
        match remove_background(path, &config).await {
            Ok(result) => {
                ...
                println!("✅ {:.0}ms (inference: {:.0}ms, {:.1}%)",
                    processing_time, timings.inference_ms, timings.inference_ratio() * 100.0);
            }
            Err(e) => {
                println!("❌ Error: {}", e);
            }
