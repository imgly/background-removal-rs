...
fn calculate_metrics(rust_mask: &[u8], js_mask: &[u8]) -> ComparisonMetrics {
    ...
    let intersection = rust_binary
        .iter()
        .zip(js_binary.iter())
        .filter(|(&r, &j)| r && j)
        .count() as f64;
    let union = rust_binary
        .iter()
        .zip(js_binary.iter())
        .filter(|(&r, &j)| r || j)
        .count() as f64;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (category, image_name) in &test_cases {
        ...
        println!(
            "   📊 FG Ratio Diff: {:.1}%",
            metrics.foreground_ratio_diff * 100.0
        );
    ...
    if successful_comparisons > 0 {
        ...
        println!(
            ...
            total_metrics.intersection_over_union / n
        );
        println!(
            "   🎯 Dice Coefficient: {:.3}",
            total_metrics.dice_coefficient / n
        );
        println!(
            "   🎯 Mean Absolute Error: {:.1}",
            total_metrics.mean_absolute_error / n
        );
        println!(
            "   🎯 Structural Similarity: {:.3}",
            total_metrics.structural_similarity / n
        );
        println!(
            "   🎯 Foreground Ratio Difference: {:.1}%",
            (total_metrics.foreground_ratio_diff / n) * 100.0
        );
