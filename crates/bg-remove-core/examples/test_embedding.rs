...
use bg_remove_core::models::{EmbeddedModelProvider, ModelManager, ModelProvider};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   Manager loaded size: {} MB",
        manager_data.len() / (1024 * 1024)
    );
    ...
    if data.starts_with(b"\x08\x01") || data.starts_with(b"ONNX") {
        ...
        println!(
            "❌ Data does not appear to be ONNX format. First 16 bytes: {:?}",
