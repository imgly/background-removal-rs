...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✅ Processing completed in {:.2}s (+ {}ms image encoding)", 
             processing_time.as_secs_f64(), encode_time_ms);
    ...
    if let Some(encode_ms) = timings.image_encode_ms {
        println!("│ Image Encode    │ {:>4}ms  │ {:>5.1}%  │", encode_ms, breakdown.encode_pct);
    }
    if other_ms > 0 {
        println!("│ Other/Overhead  │ {:>4}ms  │ {:>5.1}%  │", other_ms, breakdown.other_pct);
    }
    println!("├─────────────────┼─────────┼─────────┤");
    ...
    println!("   • Inference ratio: {:.1}% (ONNX Runtime efficiency)", timings.inference_ratio() * 100.0);
    println!("   • Image encoding: {}ms (measured separately)", encode_time_ms);
    if other_ms > 10 {
        println!("   • Overhead: {}ms ({:.1}%) - function calls, memory allocation", other_ms, breakdown.other_pct);
    }
