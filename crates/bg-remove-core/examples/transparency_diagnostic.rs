...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Foreground: {} ({:.1}%)", mask_stats.foreground_pixels, mask_stats.foreground_ratio * 100.0);
    println!("   Background: {} ({:.1}%)", mask_stats.background_pixels, mask_stats.background_ratio * 100.0);
    ...
    if Path::new(js_reference).exists() {
        ...
        if let DynamicImage::ImageRgba8(js_rgba) = js_image {
            ...
            println!("   Transparent pixels: {} ({:.1}%)", 
                transparent_count, 
                (transparent_count as f64) / (total_pixels as f64) * 100.0);
            println!("   Opaque pixels: {} ({:.1}%)", 
                opaque_count, 
                (opaque_count as f64) / (total_pixels as f64) * 100.0);
            println!("   Partial transparency: {} ({:.1}%)", 
                total_pixels - transparent_count - opaque_count,
                ((total_pixels - transparent_count - opaque_count) as f64) / (total_pixels as f64) * 100.0);
        }
    }
