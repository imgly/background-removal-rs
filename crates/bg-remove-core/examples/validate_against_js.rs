...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let js_benchmark_path = "crates/bg-remove-testing/assets/expected/benchmarks/javascript_baseline.json";
    if !Path::new(js_benchmark_path).exists() {
        println!("❌ JavaScript benchmark file not found: {}", js_benchmark_path);
        return Ok(());
    }
    
    let js_data: JavaScriptBenchmark = serde_json::from_str(
        &std::fs::read_to_string(js_benchmark_path)?
    )?;
    
    println!("📊 JavaScript Baseline Summary:");
    println!("   Portraits: {} images, avg {:.1}ms", 
        js_data.results.portraits.processed, 
        js_data.results.portraits.avg_processing_time_ms);
    println!("   Products: {} images, avg {:.1}ms", 
        js_data.results.products.processed, 
        js_data.results.products.avg_processing_time_ms);
    println!("   Complex: {} images, avg {:.1}ms", 
        js_data.results.complex.processed, 
        js_data.results.complex.avg_processing_time_ms);
    println!("   Edge cases: {} images, avg {:.1}ms\n", 
        js_data.results.edge_cases.processed, 
    ...
    for (category_name, category_data) in &categories {
        ...
        for (image_name, js_result) in &category_data.images {
            ...
            let input_path = format!("crates/bg-remove-testing/assets/input/{}/{}", category_name, image_name);
            ...
            match remove_background(&input_path, &config).await {
                Ok(rust_result) => {
                    ...
                    let rust_output_path = format!("validation_output/rust_{}_{}.png", category_name, 
                    ...
                    let rust_mask_path = format!("validation_output/rust_{}_{}_mask.png", category_name,
                        image_name.trim_end_matches(".jpg"));
                    ...
                    println!("✅ {:.0}ms (JS: {:.0}ms) | {:.1}% fg | {}x{}", 
                        rust_time_ms, js_time_ms, 
