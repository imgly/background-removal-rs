...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if Path::new(js_reference).exists() {
        ...
        if let DynamicImage::ImageRgba8(js_rgba) = js_image {
            ...
            println!(
                "     Transparent (0): {:.1}%",
                (transparent_pixels as f64) / (js_alpha_values.len() as f64) * 100.0
            );
            println!(
                "     Opaque (255): {:.1}%",
                (opaque_pixels as f64) / (js_alpha_values.len() as f64) * 100.0
            );
            println!(
                "     Partial: {:.1}%",
                (partial_pixels as f64) / (js_alpha_values.len() as f64) * 100.0
            );
        }
    }
