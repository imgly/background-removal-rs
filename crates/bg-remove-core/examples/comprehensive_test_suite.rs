...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (category, image_name) in &test_cases {
        ...
        match remove_background(&input_path, &config).await {
            Ok(result) => {
                ...
                println!("✅ {}ms | FG: {:.1}% | Range: {}-{} | Trans: {:.1}% | Opaque: {:.1}%", 
                    processing_time,
                ...
                let output_name = format!("improved_{}_{}.png", category, 
    ...
    if successful_tests > 0 {
        ...
        println!("   ⏱️  Average processing time: {}ms", total_time / successful_tests as u64);
        
        // Aggregate quality metrics
        let avg_fg_ratio = quality_metrics.iter()
            .map(|m| m.foreground_ratio)
            .sum::<f32>() / quality_metrics.len() as f32;
        let avg_transparency = quality_metrics.iter()
            .map(|m| m.transparency_ratio)
            .sum::<f32>() / quality_metrics.len() as f32;
        let avg_opacity = quality_metrics.iter()
            .map(|m| m.opacity_ratio)
            .sum::<f32>() / quality_metrics.len() as f32;
        
        println!("   🎯 Average foreground ratio: {:.1}%", avg_fg_ratio * 100.0);
        println!("   🔍 Average transparency: {:.1}%", avg_transparency * 100.0);
        println!("   🔍 Average opacity: {:.1}%", avg_opacity * 100.0);
        
        // Check for full value range usage
        let uses_full_range = quality_metrics.iter()
