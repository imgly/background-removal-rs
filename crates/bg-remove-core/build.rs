...
");
    ...
    for (model_id, model_name, variant) in &embedded_models {
        ...
        let input_shape = variant_config["input_shape"].as_array()
            .unwrap_or_else(|| panic!("input_shape not found or not an array for {model_name}:{variant}"));
        let output_shape = variant_config["output_shape"].as_array()
            .unwrap_or_else(|| panic!("output_shape not found or not an array for {model_name}:{variant}"));
        let input_name = variant_config["input_name"].as_str()
            .unwrap_or_else(|| panic!("input_name not found or not a string for {model_name}:{variant}"));
        let output_name = variant_config["output_name"].as_str()
            .unwrap_or_else(|| panic!("output_name not found or not a string for {model_name}:{variant}"));
        
        let target_size = preprocessing["target_size"].as_array()
            .unwrap_or_else(|| panic!("target_size not found or not an array for {model_name}"));
        let mean = preprocessing["normalization"]["mean"].as_array()
            .unwrap_or_else(|| panic!("normalization mean not found or not an array for {model_name}"));
        let std = preprocessing["normalization"]["std"].as_array()
            .unwrap_or_else(|| panic!("normalization std not found or not an array for {model_name}"));
...
fn load_model_config(model_name: &str) -> serde_json::Value {
    ...
    let json_content = fs::read_to_string(&model_json_path)
        .unwrap_or_else(|_| panic!("Failed to read model.json for model '{model_name}' at path: {model_json_path}"));
