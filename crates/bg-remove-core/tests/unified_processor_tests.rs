...
use bg_remove_core::{
    BackendType, BackgroundRemovalProcessor, ProcessorConfigBuilder,
...
fn save_test_image() -> PathBuf {
    ...
    test_image.save(&test_path).expect("Failed to save test image");
    test_path
}
...
async fn test_output_format_handling() -> Result<()> {
    ...
    assert!(test_path.exists(), "Test image was not created at {:?}", test_path);
