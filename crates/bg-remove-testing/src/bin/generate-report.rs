...
struct Args {
    ...
    #[arg(long, default_value = "crates/bg-remove-testing/assets/test_cases.json")]
...
impl ReportBuilder {
    ...
    async fn build_session_from_outputs(&self) -> Result<TestSession, Box<dyn std::error::Error>> {
        ...
        for (test_id, rust_output_path) in rust_outputs {
            if let Some(test_case) = self.find_test_case(&test_id) {
                ...
                let result = self.process_test_output(test_case, &rust_output_path).await?;
    ...
    fn find_output_files(&self, output_dir: &str) -> Result<Vec<(String, PathBuf)>, Box<dyn std::error::Error>> {
            ...
            if let Some(extension) = path.extension() {
                if extension == "png" || extension == "jpg" || extension == "jpeg" {
                    if let Some(file_stem) = path.file_stem() {
                        let test_id = self.extract_test_id_from_filename(&file_stem.to_string_lossy());
    ...
    async fn generate_comparison_images(&self, session: &TestSession) -> Result<(), Box<dyn std::error::Error>> {
        use bg_remove_testing::ImageComparison;
        ...
        for result in &session.results {
            let input_path = PathBuf::from(&self.args.input_dir)
                .join(&result.test_case.input_file);
            let expected_path = PathBuf::from(&self.args.expected_outputs)
            ...
            if let Some(ref rust_output_path) = result.output_path {
                if input_path.exists() && expected_path.exists() && rust_output_path.exists() {
                    ...
                    let comparison = ImageComparison::create_comparison_image(&original, &expected, &actual)?;
                    let comparison_file = comparison_dir.join(format!("{}_comparison.png", result.test_case.id));
                    comparison.save(&comparison_file)?;
                    
                    // Generate difference image
                    let diff = ImageComparison::generate_diff_image(&actual, &expected)?;
                    let diff_file = comparison_dir.join(format!("{}_diff.png", result.test_case.id));
                    diff.save(&diff_file)?;
                }
            }
        }
        
        println!("✅ Comparison images saved to: {}", comparison_dir.display());
        Ok(())
    }
}