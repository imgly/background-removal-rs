...
impl TestRunner {
    fn new(args: Args) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        if args.verbose {
            ...
            if !validation_report.is_valid() {
                eprintln!("⚠️  Found {} issues with test assets:", validation_report.total_issues());
    ...
    async fn run_single_test(&self, test_case: &bg_remove_testing::TestCase) -> Result<TestResult, Box<dyn std::error::Error>> {
    ...
    fn print_test_result(&self, result: &TestResult) {
        ...
        println!("  {} {} - {:.1}% accuracy, {}ms", 
                 status, result.test_case.id, accuracy, time);
        
        if !result.passed {
            println!("    Expected: {:.1}%, Got: {:.1}%", 
                     result.test_case.expected_accuracy * 100.0, accuracy);
        }
    }
    ...
    fn print_summary(&self, session: &TestSession) {
        ...
        println!("Passed:          {} ({:.1}%)", summary.passed_tests, pass_rate);
        println!("Failed:          {}", summary.failed_tests);
        println!("Average Accuracy: {:.1}%", summary.average_accuracy * 100.0);
        println!("Average Time:    {}ms", summary.average_processing_time.as_millis());
        println!();
        
        // Category breakdown
        println!("📊 By Category:");
        for category in &summary.categories_tested {
            let category_results: Vec<_> = session.results.iter()
            ...
            println!("  {}: {}/{} ({:.1}%)", category, category_passed, category_total, category_rate);
        }
        println!();
        ...
        if self.args.generate_report {
            println!("🌐 View detailed report: {}/comparison_report.html", self.output_dir.display());
        }
    }
}